...
class MISA():
    def __init__(self, args):
        ...
        print('args.datasetName',args.datasetName)
        ...
        with torch.no_grad():
            with tqdm(dataloader) as td:
                for batch_data in td:
                    ...
                    #如果需要返回样本结果，则收集样本ID、标签和预测结果。
                    rsr=return_sample_results
                    if return_sample_results:
                    ...
                    loss = self.criterion(outputs['M'], labels) #valid test 只计算ltask
                    eval_loss += loss.item()
